...
public class MainActivity extends AppCompatActivity {
    ...
    private void help(){new MaterialAlertDialogBuilder(this).setTitle("AppLingo 1.0")
    ...
    private void actions(){
        if(busy)return;
        String[] items={"Change selected ("+selectedApps.size()+")","Select all shown ("+shown.size()+")","Clear selection","Refresh shown languages", "Undo last change"};
        new MaterialAlertDialogBuilder(this).setTitle("App actions").setItems(items,(d,which)->{
            ...
            else if(access.canUndo())confirmUndo();else error("Nothing to undo","Your next language change will save its previous settings here.");
        }).show();
    }
