...
public class ChangeEngineTest {
 ...
 @Test public void undoDoesNotOverwriteAnExternalChange(){Memory s=new Memory();Backend b=new Backend(s);ChangeEngine.apply(requests(),b,s,(d,t)->{});b.values.put("dev.first","de");ChangeEngine.Result r=ChangeEngine.undo(b,s,(d,t)->{});assertEquals("de",b.values.get("dev.first"));assertEquals(1,r.issues.size());assertTrue(s.entries.containsKey("dev.first"));assertFalse(s.entries.containsKey("dev.second"));}
